...
fn main() -> ! {
    ...
    let _ = lstimer0.configure(config);

    let mut channel: Channel<'_, LowSpeed> =
        ledc.channel(channel::Number::Channel1, peripherals.GPIO2);
    ...
    let _ = channel.configure(channelo_config);

    loop {
        let _ = channel.start_duty_fade(0, 100, 1000);
        while channel.is_duty_fade_running() {}
        let _ = channel.start_duty_fade(100, 0, 1000);
        while channel.is_duty_fade_running() {}
    }
}
