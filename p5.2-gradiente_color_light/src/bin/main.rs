...
fn main() -> ! {
    ...
    loop {
        for duty_r in 0..=100 {
            let r = channel0.set_duty(duty_r as u8);
            match r {
                Ok(_) => {}
                Err(e) => println!("Error setting channel0 duty: {:?}", e),
            }
            for duty_g in 0..=100 {
                let r = channel2.set_duty(duty_g as u8);
                match r {
                    Ok(_) => {}
                    Err(e) => println!("Error setting channel2 duty: {:?}", e),
                }
                for duty_b in 0..=100 {
                    let r = channel4.set_duty(duty_b as u8);
                    match r {
                        Ok(_) => {}
            ...
            delay.delay_micros(20000);
        }
        for duty_r in (100..=0).rev() {
            let r = channel0.set_duty(duty_r as u8);
            match r {
                Ok(_) => {}
                Err(e) => println!("Error setting channel0 duty: {:?}", e),
            }
            for duty_g in (100..=0).rev() {
                let r = channel2.set_duty(duty_g as u8);
                match r {
                    Ok(_) => {}
                    Err(e) => println!("Error setting channel2 duty: {:?}", e),
                }
                for duty_b in (100..=0).rev() {
                    let r = channel4.set_duty(duty_b as u8);
                    match r {
                        Ok(_) => {}
                        Err(e) => println!("Error setting channel4 duty: {:?}", e),
