...
fn main() -> ! {
    ...
    // adc1 is connected to GPIO32, which is the pin we will use to read the Photoresistor value
    ...
    // ledc is connected to GPIO14, which is the pin we will use to control the LED
    let mut ledc = Ledc::new(peripherals.LEDC);
    ledc.set_global_slow_clock(LSGlobalClkSource::APBClk);

    // timer0 is connected to GPIO14, which is the pin we will use to control the LED
    ...
    // channel0 is connected to GPIO14, which is the pin we will use to control the LED
    let pin14 = Output::new(peripherals.GPIO14, Level::Low, OutputConfig::default());
    let mut channel0: Channel<'_, LowSpeed> = ledc.channel(channel::Number::Channel0, pin14);
